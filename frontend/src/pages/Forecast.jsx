...
import { usePreferences, formatRainfall } from "../context/PreferencesContext.jsx";
    ...
    <div>
      ...
      <div className="dash-header">
        <div className="dash-title">
          ...
          <div>
            <h2>{t("forecastPageTitle", { county: county.name })}</h2>
            <p className="subtitle">{t("forecastSubtitle")}</p>
          </div>
        </div>
      ...
      {status === "success" && data && (
        <ForecastDetail data={data} range={range} t={t} />
      )}
    </div>
  );
}

function ForecastDetail({ data, range, t }) {
    ...
    <>
      <div className="panel">
        <div className="panel-header">
          <span className="title">{t("rainfallTempRange", { range })}</span>
        </div>
      ...
      <div className="stat-grid" style={{ gridTemplateColumns: "repeat(3, 1fr)" }}>
        <div className="stat-card">
          <div className="label">{t("totalRainfall")}</div>
          <div className="value">{formatRainfall(totalRainfall, units)}</div>
        </div>
        <div className="stat-card">
          <div className="label">{t("avgHumidity")}</div>
          <div className="value">{avgHumidity}%</div>
        </div>
        <div className="stat-card">
          <div className="label">{t("peakRainDay")}</div>
          <div className="value">{t("day", { n: peakIndex + 1 })}</div>
        </div>
      </div>
    </>
  );
